                                ...
                                google::protobuf:: Closure* done)
{
    ...
    //std::cout<<"already send str:"<<send_rpc_str<<std::endl;
    ...
    //if (!response->ParseFromString(conn->ReadString()))
    if (!response->ParseFromArray(conn->ReadBuffer(), conn->ReadString().size()))
    {
        conn->Close();
        char errtxt[512] = {0};
        sprintf(errtxt, "parse error! response_str:%s", conn->ReadBuffer());
